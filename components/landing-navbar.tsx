...
/**
 * LandingNavbar - Public navbar for landing pages (explore, leaderboard, projects, etc.)
    ...
    <nav className="border-b border-zinc-200 dark:border-zinc-800">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6">
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Desktop Connect Wallet / Dashboard */}
          <div className="hidden sm:flex sm:items-center sm:gap-3">
            {authenticated ? (
              <Link
                href="/dashboard"
                className="rounded-lg bg-[#FFBF00] px-4 py-2 text-sm font-medium text-black transition-opacity hover:opacity-90"
              >
                Dashboard
              </Link>
            ) : (
          ...
          <div className="flex flex-col gap-4">
            ...
            {authenticated && (
              <Link
                href="/dashboard"
                onClick={() => setIsOpen(false)}
                className="rounded-lg bg-[#FFBF00] px-4 py-2 text-center text-sm font-medium text-black transition-opacity hover:opacity-90"
              >
                Go to Dashboard
              </Link>
            )}
